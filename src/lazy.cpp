...
template<>
void LazyTSP<Point2DPolynom>::savePaths(const FileStruct file, const std::deque<std::tuple<int,int>> &selectedPaths) {
  INFO("Saving paths");
  std::ofstream fileStream{file.fileName.c_str()};
  if (!fileStream.good()) {
    std::stringstream message;
  ...
  if (fileStream.is_open()) {
    ...
    if (file.type == Obj) {
      ...
      for (auto &pair : selectedPaths) {
        auto [ first, second ] = pair;
        DistanceHolder<Point2DPolynom> &holder{this->neighboringMatrix(first, second)};

        unsigned startingInd{vertexInd};
        auto plan{holder.Node1->Position.SampleTrajectory(holder.Node2->Position, this->problem.CtrlInterval)};
        for (int m{0}; m < plan.size(); ++m) {
          Point2DPolynom actPoint{plan[m]};

          fileStream << "v" << DELIMITER_OUT;
          Point2DPolynom temp{actPoint / problem.Env.ScaleFactor}; 
          temp.PrintPosition(fileStream);
          fileStream << "\n";
        }
        vertexInd += plan.size();
...
template<> 
double LazyTSPBase<Point2DPolynom>::getApproxDistance(Node<Point2DPolynom> &start, Node<Point2DPolynom> &goal) {
  return start.Position.EuclideanDistance(goal.Position);
}
