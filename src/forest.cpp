...
template<>
bool SpaceForestBase<Point2DPolynom>::checkExpandedTree(Node<Point2DPolynom> *expanded, Point2DPolynom* newPoint, flann::Matrix<float> &matrix) {
  double parentDistance{expanded->Position.EuclideanDistance(*newPoint)};
  Tree<Point2DPolynom> *expandedTree{expanded->SourceTree};
  int expandedRootID{expandedTree->Root->ID};
  Point2DPolynom newPosition{newPoint->GetPosition()[0], newPoint->GetPosition()[1]};
  ...
  for (int i{0}; i < neighbours; ++i) {
    int neighID{indRow[i]};
    Node<Point2DPolynom> *neighbor{&(expandedTree->Leaves[neighID])};
    
    // distance betweeen positions only -- checking EUCLIDEAN DISTANCE not the trajectory cost
    double realDist{neighbor->Position.EuclideanDistance(*newPoint)};
    Point2DPolynom neighPoint{neighbor->Position};
    Point2DPolynom neighPosition{neighPoint.GetPosition()[0], neighPoint.GetPosition()[1]};
    
    if (realDist < (parentDistance - SFF_TOLERANCE)) {
      // local planner checking ONLY for position (not trajectory between points)
      double parts{realDist / problem.CollisionDist};
      bool isFree{true};
      PointVector2D direction{newPosition, neighPosition};
      Point2DPolynom position;
      for (int index{1}; index < parts && isFree; ++index) {
        position.SetPosition(newPosition[0] + direction[0] * index / parts, newPosition[1] + direction[1] * index / parts);
...
template<> 
bool SpaceForestBase<Point2DPolynom>::checkOtherTrees(Node<Point2DPolynom> *expanded, Point2DPolynom* newPoint, flann::Matrix<float> &matrix, bool &solved) {
  int expandedRootID{expanded->SourceTree->Root->ID};
  std::vector<std::vector<int>> indices;
  std::vector<std::vector<float>> dists;
  ...
  double checkDist{FLANN_PREC_MULTIPLIER * this->problem.DistTree};
  for (int j{0}; j < this->trees.size(); ++j) {
    Tree<Point2DPolynom> &tree{this->trees[j]};
    int neighbourRootID{tree.Root->ID};
    if (neighbourRootID == expandedRootID) {
      continue;
    } 
    ...
    for (int i{0}; i < neighbours; ++i) {
      int neighID{indRow[i]};
      Node<Point2DPolynom> *neighbour{&(tree.Leaves[neighID])};
      ...
      if (realDist < (this->problem.DistTree - SFF_TOLERANCE)) {   // if realDist is bigger than tree distance, it might get expanded later
        ...
        if (this->problem.HasGoal && neighbour->Position == this->problem.Goal) {
          ...
          std::deque<DistanceHolder<Point2DPolynom>> &borderPoints{this->borders(neighbourRootID, expandedRootID)};
          DistanceHolder<Point2DPolynom> holder{neighbour, expanded};
          if (std::find(borderPoints.begin(), borderPoints.end(), holder) == borderPoints.end()) {  // is it necessary? maybe use sorted set?
            this->borders(neighbourRootID, expandedRootID).push_back(holder);
          }
        }
